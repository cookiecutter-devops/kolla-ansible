...
- name: Check whether KVM is enabled in kernel
  ansible.builtin.debug:
    msg: "KVM kernel support is enabled"
  when: ansible_facts.kernel_config.kvm | default('n') in ['y', 'm']

- name: Check bridge netfilter
  ansible.builtin.assert:
    that:
      - ansible_facts.kernel_config.bridge_netfilter | default('n') in ['y', 'm']
    fail_msg: "Kernel does not support bridge netfilter"
