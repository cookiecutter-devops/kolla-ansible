...
# github: https://github.com/melmorabity/ansible-ftp.git
...
def _ftp_upload(module, session, src, dest, top_dir=True):
    ...
    if results["changed"]:
        ...
        try:
            ...
            module.fail_json(
                msg="Unable to upload source {} to destination {}: {}".format( src, dest, to_native(ex) )
            )
...
def _ftp_download(module, session, src, dest, top_dir=True):
    ...
    try:
        ...
        module.fail_json(
            msg="Unable to get status for source {}: {}".format( src, to_native(ex) )
        )
    ...
    if dest_exists:
        ...
        if not dest_is_dir and not os.path.isfile(dest):
            module.fail_json( msg="Destination {} is not a regular file or a " "directory".format(dest) )
        ...
        if tmp_dest:
            ...
            if not os.path.isdir(tmp_dest):
                if os.path.exists(tmp_dest):
                    module.fail_json(
                        msg="{} is a file but should be a directory".format( tmp_dest )
                    )
                else:
                    module.fail_json( msg="{} directory does not exist".format(tmp_dest) )
...
def main():
    ...
    with contextlib.closing(FTP()) as session:
        try:
            ...
            module.fail_json( msg="Unable to connect to FTP server: {}".format(to_native(ex)) )

        results = None
        direction = module.params["direction"]
        if direction == "push":
            results = _ftp_upload( module, session, module.params["src"], module.params["dest"] )
        elif direction == "pull":
            results = _ftp_download( module, session, module.params["src"], module.params["dest"] )
